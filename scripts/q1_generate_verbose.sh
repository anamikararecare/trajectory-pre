#!/usr/bin/env bash
# Verbose Q1 generator: loads .env and reports shard, conversation, response,
# timing, word-count, retry, and completion progress.
set -Eeuo pipefail
...
python -u -m src.q1.q1_verbose_cli generate \
  --protocol configs/q1_generation_protocol.yaml \
  --run-id "$q1_run_id" \
